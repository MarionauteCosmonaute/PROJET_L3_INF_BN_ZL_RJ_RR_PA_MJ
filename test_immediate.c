...
int main ()
{
    ...
    int rotval=rand()%16;
    uint32_t immediate=rand() % 128;
    int bit_I;

    for(int opeCode=0; opeCode<16;opeCode++)
    {
        operation= squeleton | (opeCode <<21) | (1<<25) | (rotval<<8)|immediate;
        printf("OpCode = %d\n", opeCode );
        printf("rotval = %d\n",rotval);
        printf("immediate=%d\n",immediate);
        ...
        uint64_t temp;
        uint32_t res2 = arm_read_register(p, Rd_num);   // Resultat du programme
